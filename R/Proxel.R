...
#' of a product with zero repair time, along with the average warranty cost.
#'
...
Proxel <- function(state, W, model, delta, tol, alpha, K) {
  tt <- delta
  steps <- W / delta
  ...
  funsext <- function(s, model, delta, tt, alpha, W, K) {
    pL <- nextLevel(prox[s, ], model, delta, tt, alpha, W, K)
